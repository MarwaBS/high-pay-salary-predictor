...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--dry-run", action="store_true", help="Load data and engineer features, then exit without training."
    )
    p.add_argument(
        "--tune", action="store_true", help="Re-run Optuna HPO search before training (overrides config params)."
    )
    return p.parse_args()
...
def main() -> None:
    ...
    logger.info(
        "Target log1p-transformed. y_train range: [%.2f, %.2f]", float(y_train_log.min()), float(y_train_log.max())
    )
    ...
    pi_coverage = float(((residuals_dollar >= pi_offset_10) & (residuals_dollar <= pi_offset_90)).mean())
    pi_width = float(np.median(pi_offset_90 - pi_offset_10))
    logger.info(
        "Empirical 80%% PI  offset_10=$%d  offset_90=$%d  coverage=%.1f%%  width=$%d",
        int(pi_offset_10),
        int(pi_offset_90),
        pi_coverage * 100,
        int(pi_width),
    ...
    perm_importance = {
        ...
        for feat, m, s in zip(FEATURES_FULL, perm.importances_mean, perm.importances_std, strict=True)
    }
    # Log top features
    sorted_feats = sorted(perm_importance, key=lambda k: -perm_importance[k]["mean"])
    for feat in sorted_feats:
        logger.info(
            "  perm_imp  %-25s  mean=%.4f  std=%.4f", feat, perm_importance[feat]["mean"], perm_importance[feat]["std"]
        )
