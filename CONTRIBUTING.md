...
- **Authorship.** Commits are authored by the repository owner. The one
  exception is `dependabot[bot]`, whose automated dependency PRs land under the
  bot's own identity; `.github/dependabot.yml` restricts it to patch and minor
  bumps across all three ecosystems, so a major-version change is always a
  human decision. `numpy`, `scikit-learn` and `xgboost` are excluded entirely:
  they are pinned to the versions `models/tuning_study.json` was scored under, so
  bumping one means re-running `scripts/tune.py` in the same change.
