...
- Retrain the model with `make model` and verify tests still pass.
- If R² changes significantly, update the `test_r2_above_floor` threshold in `tests/test_pipeline.py`.
...
- **Do not commit large data files.** The pre-commit hook blocks files > 5 MB.
